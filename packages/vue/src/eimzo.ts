        ...
        new Promise((resolve, reject) => {
          EIMZOClient.checkVersion(
            (major: string, minor: string) => {
              ...
              const installedVersion = Number.parseInt(major) * 100 + Number.parseInt(minor)

              if (installedVersion < newVersion) {
                reject(new Error(getErrorMessage('UPDATE_APP')))
              }
              else {
                resolve({ major: Number.parseInt(major), minor: Number.parseInt(minor) })
              }
            },
        ...
        new Promise((resolve, reject) => {
          EIMZOClient.loadKey(
            ...
            (_e: unknown, r: string | null) => {
              if (r) {
                if (r.includes('BadPaddingException')) {
