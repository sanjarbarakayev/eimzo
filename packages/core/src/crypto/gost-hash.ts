...
export class GostHash {
  private hash: number[] = Array.from({ length: 8 })
  private sum: number[] = Array.from({ length: 8 })
  private message: number[] = Array.from({ length: 32 })
  ...
  constructor() {
    ...
    for (let j = 0; j < 4; j++) {
      this.gostSbox[j] = Array.from<number>({ length: 256 })
    }
  }
  ...
  private str2bytes(str: string): number[] {
    const arr: number[] = Array.from({ length: str.length })
  ...
  private hexStr2bytes(str: string): number[] {
    ...
    const arr: number[] = Array.from({ length: str.length / 2 })
  ...
  private gostBlockCompress(block: number[]): void {
    const key: number[] = Array.from({ length: 8 })
    const u: number[] = Array.from({ length: 8 })
    const v: number[] = Array.from({ length: 8 })
    const w: number[] = Array.from({ length: 8 })
    const s: number[] = Array.from({ length: 8 })
  ...
  private gostUpdate(msg: number[], size: number): void {
    ...
    const msg32: number[] = Array.from({ length: 8 })
  ...
  gosthash(value: string): string {
    const rx: string[] = Array.from({ length: 8 })
