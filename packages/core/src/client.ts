...
function splitKeep(str: string, splitter: string | RegExp, ahead: boolean = false): string[] {
  const result: string[] = []
...
function parsePfxCertificate(el: PfxCertificateRaw): PfxCertificate | null {
...
function parseFtjcCertificate(el: FtjcTokenRaw): FtjcCertificate | null {
        ...
        if (data.success === true) {
          if (data.major && data.minor) {
            const installedVersion
              = Number.parseInt(data.major) * 100 + Number.parseInt(data.minor)
